...
const files = ['UserRepository.ts', 'DealRepository.ts'];
...
for (const file of files) {
    ...
    if (c.includes(`export class ${file.replace('.ts', '')} {`)) {
        c = c.replace(new RegExp(`export class ${file.replace('.ts', '')} \\{`), `export class ${file.replace('.ts', '')} extends BaseRepository<any> {\n  protected collectionName = "${collectionName}";\n  protected entityType = "${entityType}";\n`);
        c = extractMethods(c);
